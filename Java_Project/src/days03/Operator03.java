...
public class Operator03 {
	...
	public static void main(String[] args) {
		...
		// String을 대상으로 할 수 있는 비교연산은 == 와 != 둘밖에 없으며, 이마저도 우리가 원하는 결과로는 연산되지 않습니다.
		// == 와 != 연산이 에러 없이 기능은 하지만 이도 사용은 하지 않습니다.
		
		// s1과 s2는 String 자료형이며, 다른 기본 자료형과 다르게 이 두 변수가 직접 문자들을 저장하고 있지 않습니다.
		// 문자들은 별도의 장소에 저장되고, 변수는 저장 위치값만 저장하고 있습니다.
		// 따라서 변수들간의 비교 (s1>s2)는 저장 위치정보의 비교이므로 연산이 의미가 있지도 않고 가능하지도 않습니다.
		
		// String 자료간의 비교는 별도로 만들어져 있는 도구를 이용합니다.
		// String 의 크다 작다를 판단 해주는 compareTo()
		// String 의 같다 다르다를 판단 해주는 equals()
		
		// compareTo 를 사용한 결과는 앞 항의 글자에서 뒷 항의 글자를 뺀 결과가 정수형태로 나오는 형태입니다.
		// 앞글자가 크면 양수, 뒷들가가 크면 음수의 값이 두 글자의 아스키코드의 차 값으로 계산 됩니다.
