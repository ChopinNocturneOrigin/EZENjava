...
public class Variable06 {
	...
	public static void main(String[] args) {
		// 정수형 : byte(1), short(2), int(4 - 기본형) -2147483648 ~ 2147483647, long(8)
		// 실수형 : float(4), double(8 - 기본형)
		
		// 기본형이란 별도의 표시없이 쓰여지는 정수나 실수가 int와 double로 인식되어 사용 된다는 뜻입니다.
		// 100(int형 자료), 100L(long형 자료), 123.123(double형 자료), 123.123f(float형 자료)
		// byte b = 1234; // 자료손실에 의해서 에러
		// float f = 123.12; // 자료손실에 의해서 에러
		...
		// 변수는 선인 이후라도, 값이 대입되지 않으면 사용이 불가합니다.
		int v4;
		...
		// char형 데이터는 컴퓨터 저장장치에 저장될 때 특별한 이진수 코드로 저장됩니다.
		// 문자를 저장하기 위한 코드체계에 의해 조합된 코드입니다.
		// 이 이진수를 십진수로 변환하면 정수로 변환이 가능합니다.
		// 그 결과 정수와 문자간에는 약간이 호환성이 발생됩니다.
		...
		System.out.println();
		for(int i=0;i<256;i++) {
			System.out.printf("'%c':%3d\t",i,i);
			if(i%10==9) System.out.println();
		}
		
		// 실수형 변수의 초기화
		// 자바에서 모든 실수들은 별도의 표기가 없으면, 기본적으로 double 타입으로 인식합니다.
		// float 형으로 값을 저장하기 위해서는 float 형 변수를 선언하고 float 형 데이터를 저장합니다.
		// float 형 변수에 double 형 데이터 입력 -> 에러
		// float f1 = 1.123; // 에러
		// float 형 변수에 float 형 데이터 입력 -> 정상실행
		float f2 = 1.123f; // 또는 float f2 = 1.123F;
		
		// float 형 변수에 double 형 변수값을 저장하는 방법 : 캐스팅 연산 실행
		double f3 = 1.123;
		float f4 = (float)f3;
		
		// 용량이 큰 자료형(double)에 작은 자료형(float) 데이터 대비 -> 정상실행
		float f5 = 123.123f;
		double f6 = f5; // 캐스팅 연상이 필요 없습니다.
		...
		byte b2 = 123; // 일반값에 의한 대입은 캐스팅이 필요 없습니다. (정수 데이터에 한해서만 허용)
		a = b2; // 용량이 작은 자료형을 큰 자료형에 대입은 정상실행
		
		// 용량이 큰 변수값을 용량이 작은 변수값으로 넣으려고 하면 캐스팅 연산 필요
		// 반대의 경우는 문제 없이 모든 실행이 가능합니다.
