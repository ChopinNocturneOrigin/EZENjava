...
// default   : 접근 지정자를 지정하지 않으면 자동으로 갖게되는 접근 지정자이며,
//             같은 패키지내에서 자유롭게 접근이 가능합니다.

class Account { // 은행 계좌 클래스
	private double balance; // 계좌 잔액이 저장될 멤버 변수
	
	// 클래스의 멤버변수와 멤버메서드는 접근 지정자(private, public, protected)에 의해서
	// 외부에서의 접근을 조절할 수 있습니다.
	// 접근지정자를 사용하지 않는 경우 기본지정으로 public이 설정되며, 클래스 외부에서 직접접근이 가능합니다.
	// 이는 멤버변수의 값이 무분별하게 수정될 수 있다는 뜻 입니다.
	// 이를 방지하고 무분별한 접근을 막기위해 멤버변수에 private를 지정합니다.
	// private로 지정된 멤버는 자기자신이 속한 class 내부의 멤버메서드를 통해서만 접근 가능합니다.
	// 멤버메서드는 대개 public으로 지정되며, 외부에서 자유로운 접근이 가능합니다.
